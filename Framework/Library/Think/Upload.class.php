...
class Upload {
    ...
    private $config = array(
        ...
        'relativePath' => '/Public/Upload/', //相对路径
        'savePath' => '', //保存路径
        'saveName' => array('trim', array('__MD5__')), //上传文件命名规则，[0]-函数名，[1]-参数，多个参数使用数组
    ...
    public function __construct($config = array(), $driver = '', $driverConfig = null) {
        /* 获取配置 */
        $this->config['rootPath'] = PUBLIC_PATH . 'Upload/';
    ...
    /**
     * 上传文件
     * @param  string|array $files 文件信息数组，通常是 $_FILES数组
     * @return bool|array
     */
    ...
    private function getSaveName($file) {
        ...
        if (empty($rule)) { //保持文件名不变
            ...
            $savename = $this->getName($rule, $file);
            if (empty($savename)) {
                $this->error = '文件命名规则错误！';
                return false;
            }
     ...
     * @return string
     */
    private function getSubPath($file) {
        $subpath = '';
        $rule = $this->subName;
     ...
     */
    private function getName($rule, $file) {
        $name = '';
        if (is_array($rule)) { //数组规则
            $func = $rule[0];
            $param = (array) $rule[1];
            foreach ($param as &$value) {
                $value = str_replace('__FILE__', $file['name'], $value);
                $value = str_replace('__MD5__', $file['md5'], $value);
                $value = str_replace('__SHA1__', $file['sha1'], $value);
            }

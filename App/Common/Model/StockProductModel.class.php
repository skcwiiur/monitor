...
class StockProductModel extends BaseModel {
    ...
    public function searchByPage($search) {
        ...
        if ($page_size > 0) {
            ...
            //数据查询
            $list = $this->field("t.*, t1.sku AS product_id_l, t2.name AS wh_id_l")
                ->alias('t')
                ->join($join)
                ->where($where)
                ->order("t.id desc")
     ...
     * @return array
     */
    public function change($product_id, $wh_id, $num, $type, $biz_id) {
        $product_id = (int)$product_id;
        $wh_id = (int)$wh_id;
        $num = (int)$num;
        $type = (int)$type;
        $biz_id = (int)$biz_id;
        if (empty($product_id) || empty($wh_id) || empty($num) || empty($type) || empty($biz_id)) {
        ...
        if ($product['is_delete'] == 1) {
            $this->rollback();
            return formatResult(OP_FAIL, [], "{$product['sku']}货品已被删除！");
        }
        ...
        if (empty(STOCK_P_BIZ_TABLES[$type])) {
            $this->rollback();
            return formatResult(OP_FAIL, [], "类型错误！");
        }
        $biz_table = STOCK_P_BIZ_TABLES[$type];
        $biz = M()->table($biz_table)->find($biz_id);
        if (empty($biz)) {
            $this->rollback();
            return formatResult(OP_FAIL, [], "业务数据关联失败！");
        }
        $id = $this->where(['product_id' => $product_id, 'wh_id' => $wh_id])->getField('id');
        if (empty($id)) {
            // 新增记录
            if ($num < 0) {
                $this->rollback();
                return formatResult(OP_FAIL, [], "{$product['sku']}库存变更失败，库存小于0！");
            }
            $this->addData([

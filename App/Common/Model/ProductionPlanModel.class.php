...
class ProductionPlanModel extends BaseModel {
    ...
    public function searchByPage($search) {
        ...
        $where = [];
        $join = [
            " LEFT JOIN wms_production_batch on wms_production_batch.id=t.batch_id",
        ...
        if (isset($search['batch_id']) && $search['batch_id'] !== '') {
            $where['t.batch_id'] = trim($search['batch_id']);
        }
        ...
        if ($page_size > 0) {
            ...
            //数据查询
            $list = $this->field("t.*, wms_production_batch.bh as batch_id_l, wms_warehouse.name as wh_id_l, auc.username as create_uid_f, auu.username as update_uid_f, FROM_UNIXTIME(t.create_time) AS create_time_f, FROM_UNIXTIME(t.update_time) AS update_time_f")
                ->alias('t')
                ->join($join)
                ->where($where)
                ->order("t.id desc")
    ...
    public function getRecord(int $id) {
        ...
        $record = $this->field("t.*, wms_production_batch.bh as batch_id_l, wms_warehouse.name as wh_id_l, auc.username as create_uid_f, auu.username as update_uid_f, FROM_UNIXTIME(t.create_time) AS create_time_f, FROM_UNIXTIME(t.update_time) AS update_time_f")
            ->alias('t')
            ->join($join)
            ->where(['t.id' => $id])
            ->find();

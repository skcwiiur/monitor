...
class AttrValueModel extends BaseModel {
  ...
  public function searchByPage($search) {
    ...
    $where = [];
    $join = [
      " LEFT JOIN wms_attr_dic as t1 on t1.id=t.attr_id",
    ...
    if (isset($search['attr_id']) && $search['attr_id'] !== '') {
      $where['t.attr_id'] = trim($search['attr_id']);
    }
    ...
    if (isset($search['value']) && $search['value'] !== '') {
      ...
      $where['t.value'] = ['LIKE', "%{$search['value']}%"];
    }
    if ($page_size > 0) {
      //分页信息
      ...
      //数据查询
      $list = $this->field("t.*, t1.name as attr_id_l, auc.username as create_uid_f, auu.username as update_uid_f, FROM_UNIXTIME(t.create_time) AS create_time_f, FROM_UNIXTIME(t.update_time) AS update_time_f")
        ->alias('t')
        ->join($join)
        ->where($where)
        ->order("t.id desc")

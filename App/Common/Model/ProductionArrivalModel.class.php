...
class ProductionArrivalModel extends BaseModel {
  /**
   * 列表查询
   * @param array $search
   * @return array
  ...
  public function searchByPage($search) {
    ...
    if (isset($search['arrival_time']) && $search['arrival_time'] !== '') {
      $where['t.arrival_time'] = trim($search['arrival_time']);
    }
    ...
    if (isset($search['state']) && $search['state'] !== '') {
      $search['state'] = trim($search['state']);
    ...
    if ($page_size > 0) {
      ...
      //数据查询
      $list = $this->field("ar.bill_sn,ar.id as relation_id,t.*, wms_warehouse.name as wh_id_l, wms_admin_users.username as review_uid_l, auc.username as create_uid_f, auu.username as update_uid_f, FROM_UNIXTIME(t.create_time) AS create_time_f, FROM_UNIXTIME(t.update_time) AS update_time_f")
        ->alias('t')
        ->join($join)
        ->where($where)
        ->order("t.id desc")
  ...
  public function getRecord(int $id) {
    ...
    $record['arrival_time_f'] = format_time($record['arrival_time']);
    return $record;
  }
}

...
class MaterialMakeInventoryDetailModel extends BaseModel {
    /**
     * 列表查询
     * @param array $search
     * @return array
    ...
    public function searchByPage($search) {
        ...
        $where = [];
        $join = [
            " INNER JOIN wms_material_make_inventory as t0 on t0.id=t.pid",
            " LEFT JOIN wms_material on wms_material.id=t.material_id",
        ...
        if ($page_size > 0) {
            ...
            //数据查询
            $list = $this->field("t.*, wms_material.sku as material_id_l, auc.username as create_uid_f, auu.username as update_uid_f, FROM_UNIXTIME(t.create_time) AS create_time_f, FROM_UNIXTIME(t.update_time) AS update_time_f, t0.bh,t0.state AS p_state")
                ->alias('t')
                ->join($join)
                ->where($where)
                ->order("t.id desc")

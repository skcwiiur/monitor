...
class PurchasePlanMaterialDetailModel extends BaseModel {
    /**
     * 列表查询
     * @param array $search
     * @return array
    ...
    public function searchByPage($search) {
        ...
        $where = [];
        $join = [
            " LEFT JOIN wms_purchase_plan_material as t1 on t1.id=t.plan_id",
        ...
        if ($page_size > 0) {
            ...
            //数据查询
            $list = $this->field("t.*, t1.bh as plan_id_l, t1.state as plan_state, t2.sku as material_id_l, auc.username as create_uid_f, auu.username as update_uid_f, FROM_UNIXTIME(t.create_time) AS create_time_f, FROM_UNIXTIME(t.update_time) AS update_time_f")
                ->alias('t')
                ->join($join)
                ->where($where)
                ->order("t.id desc")
            ...
            //无分页查询
            $list = $this->field("t.*, t1.bh as plan_id_l, t2.sku as material_id_l, auc.username as create_uid_f, auu.username as update_uid_f, FROM_UNIXTIME(t.create_time) AS create_time_f, FROM_UNIXTIME(t.update_time) AS update_time_f")
                ->alias('t')
                ->join($join)
                ->where($where)
                ->order("t.id desc")

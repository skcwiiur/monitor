...
class ProductionDiscardedModel extends BaseModel {
    /**
     * 列表查询
     * @param array $search
     * @return array
    ...
    public function searchByPage($search) {
        ...
        $join = [
            ...
            " LEFT JOIN wms_admin_users AS t2 ON t2.id=t.finish_uid",
        ...
        if ($page_size > 0) {
            ...
            //数据查询
            $list = $this->field("t.*, t1.name AS wh_id_l, t2.username AS finish_uid_l, auc.username AS create_uid_f, auu.username AS update_uid_f, FROM_UNIXTIME(t.create_time) AS create_time_f, FROM_UNIXTIME(t.update_time) AS update_time_f")
                ->alias('t')
                ->join($join)
                ->where($where)
                ->order("t.id desc")
    ...
    public function getRecord(int $id) {
        ...
        $record = $this->field("t.*, t1.name AS wh_id_l, t2.username AS finish_uid_l, auc.username AS create_uid_f, auu.username AS update_uid_f, FROM_UNIXTIME(t.create_time) AS create_time_f, FROM_UNIXTIME(t.update_time) AS update_time_f")
            ->alias('t')
            ->join($join)
            ->where(['t.id' => $id])
            ->find();

...
class GoodsModel extends BaseModel {
    ...
    public function searchByPage($search) {
        ...
        $where = [];
        $join = [
            " LEFT JOIN wms_category as t1 on t1.id=t.cat_id",
            " LEFT JOIN wms_brand as t2 on t2.id=t.brand_id",
        ...
        if (isset($search['goods_sn']) && $search['goods_sn'] !== '') {
            $search['goods_sn'] = trim($search['goods_sn']);
            $where['t.goods_sn'] = ['LIKE', "%{$search['goods_sn']}%"];
        }
        if (isset($search['goods_name']) && $search['goods_name'] !== '') {
            $search['goods_name'] = trim($search['goods_name']);
            $where['t.goods_name'] = ['LIKE', "%{$search['goods_name']}%"];
        }
        ...
        if ($page_size > 0) {
            ...
            //数据查询
            $list = $this->field("t.*, t1.name as cat_id_l, t2.name as brand_id_l, auc.username as create_uid_f, auu.username as update_uid_f, FROM_UNIXTIME(t.create_time) AS create_time_f, FROM_UNIXTIME(t.update_time) AS update_time_f")
                ->alias('t')
                ->join($join)
                ->where($where)
                ->order("t.id desc")

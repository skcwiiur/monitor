...
class StockMaterialLogModel extends BaseModel {
    /**
     * 列表查询
     * @param array $search
     * @return array
    ...
    public function searchByPage($search) {
        ...
        if ($page_size > 0) {
            ...
            //数据查询
            $list = $this->field("t.*, t1.sku AS material_id_l, t2.name AS wh_id_l, auc.username AS create_uid_f, FROM_UNIXTIME(t.create_time) AS create_time_f")
                ->alias('t')
                ->join($join)
                ->where($where)
                ->order("t.id desc")

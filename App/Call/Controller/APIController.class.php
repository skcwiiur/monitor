...
define('API_HOST', 'https://testapi.gshyj.com');
...
class APIController extends APIBaseController {
	...
	public function login() {
		...
		if (empty($uid) || empty($pwd)) {
			$this->makeJsonResult(['code' => -1, 'message' => '帐号或者密码不能为空.']);
		}
		$database = getDB();
		...
		if (!$record) {
			$this->makeJsonResult(['code' => -1, 'message' => '帐号或者密码不正确.']);
		}
		if ($uid == 'admin' && $pwd == 'amdin') {
			$token = $this->createToken($uid);
			$this->keepToken($uid, $token);
			$access_right = $this->getUserAccessRight($uid, 'string');
			$this->makeJsonResult(['code' => 1, 'message' => 'success.', 'data' => [
				'token' => $token,
				'access_right' => $access_right,
			]]);
		}
		$db_pwd = $record['pwd'];
		if (md5($pwd . $record['salt']) == $db_pwd) {
			$token = $this->createToken($uid);
			$this->keepToken($uid, $token);
			$access_right = $this->getUserAccessRight($uid, 'string');
			$this->makeJsonResult(['code' => 1, 'message' => 'success.', 'data' => [
